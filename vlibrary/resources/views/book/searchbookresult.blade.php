...
<script>
	...
	$(document).ready(function() {
    	...
    	$("#bModalSend").click(function(event) {	   
    	     alert(ownedBookListId + "and borrerid " + borrowerId);
    	
    	     if ((isNaN(ownedBookListId) || isNaN(borrowerId))) {
    	     	alert ("you cant see details you are not logged in");
    	     } else {    	     
    	     	$.post('http://localhost/laravel/vlibrary/public/index.php/ajax/postmessage',{ 
    	     		'ownedBookListId' : ownedBookListId, 'borrowerId' : borrowerId}, onSuccess);
...
<div class="container">
	...
	<?php
    	...
    	echo "<br> <br>";
    	echo "<div id='curUser' value='$user->id'> </div>";
    //	var_dump($name);
...
<div id='cssmenu'>
	...
	<h2 align="center">Vlibrary search result</h2>
		    ...
		    <tbody>
		      ...
		      @for ($i = 0; $i < count($allInformation); $i++)
			      ...
			      <tr>
			        ...
			        <td>{{ $allInformation[$i]['userFullName'] }}</td>
			        <td>{!! $allInformation[$i]['userPhoneNo'] . '<br>' . $allInformation[$i]['userMobileNo'] !!}</td>
			        <td>{{ $allInformation[$i]['userIsAvailable'] }}</td>
			        <td>{{ $allInformation[$i]['publishers'] }}</td>
			        <td>
				        <button id="{{ $allInformation[$i]['ownedBookListId'] }}" 
				  		data-toggle="modal" class="baccept" data-target="#myModal"
				  		value="{{ $allInformation[$i]['ownedBookListId'] }}">Details</button>
			  		</td>
		  ...
		  <div class="modal-dialog">
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
				...
				<button id="bModalSend">Send Messege</button>
		      </div>
